...
| --- | --- |
| Acute chorioamnionitis (staging/grading) | MIR findings + FIR findings combined into one descriptive block (e.g. "-- Acute chorionitis [with fetal inflammatory response in chorionic plate vessels]:" with MIR/FIR stage+grade lines); MIR stage/grade + FIR stage/grade selects; optional bacteria/Gram/GMS. Isolated FIR without MIR prints as a standalone diagnosis. The subchorionitis/chorionitis/chorioamnionitis info boxes define the MIR/FIR stages & grades (manuscript Table 2) and link to the atlas's acute chorioamnionitis chapter. No-IAI comment via `clinicalIAI`. |
