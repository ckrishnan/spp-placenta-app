...
- `src/lib/atlas-catalog.ts` (server-only) — scans the folder tree with `fs`, merges curated metadata from the manifest (matched by `imageUrl`), and returns `AtlasChapter[]` (`{ folder, title, images }`). Filenames are slugified into unique ids (deduped so `uc hypercoiled 2.jpg` ≠ `uc hypercoiled-2.jpg`) and title-cased into labels when no curated entry exists.
...
| --- | --- |
| Acute chorioamnionitis (staging/grading) | `AC` pattern; MIR stage/grade + FIR stage/grade selects; descriptive title block; optional bacteria/Gram/GMS. No-IAI comment via `clinicalIAI`. |
